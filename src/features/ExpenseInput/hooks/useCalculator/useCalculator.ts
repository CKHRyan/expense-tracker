...
export const useCalculator = ({ onChange }: Params): CalculatorInterface => {
  ...
  const del = () =>
    setDisplayValue((_displayValue) => {
      if (isEndWithOperator(_displayValue) || isEndWithDecimal(_displayValue)) {
      ...
      if (hasOperator) return replaceSubValue(_displayValue, updatedFocusValue);
  ...
  const appendOperator = (op: CalculatorOpKey) => {
    if (hasOperator) {
      calculate();
    }
    setDisplayValue((_displayValue) => {
  ...
  const setCalculatorValue = (value: number) =>
